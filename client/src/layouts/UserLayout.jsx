...
import { HiHome, HiLogout, HiArrowLeft, HiMenuAlt2, HiUser, HiHeart, HiClipboardList, HiEye, HiKey, HiChat } from 'react-icons/hi';
...
const UserLayout = () => {
  ...
  const links = [
    ...
    { to: '/dashboard/properties', icon: <HiHome />, label: 'My Properties' },
    { to: '/dashboard/wishlist', icon: <HiHeart />, label: 'Saved Properties' },
    { to: '/dashboard/recently-viewed', icon: <HiEye />, label: 'Recently Viewed' },
    { to: '/dashboard/bookings', icon: <HiClipboardList />, label: 'My Bookings' },
    { to: '/dashboard/messages', icon: <HiChat />, label: 'Messages' },
    { to: '/dashboard/change-password', icon: <HiKey />, label: 'Change Password' },
  ];
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0f172a' }}>
      <aside style={{ width: sidebarOpen ? 260 : 0, overflow: 'hidden', background: 'rgba(15,23,42,0.95)', borderRight: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', transition: 'width 0.3s ease', position: 'fixed', top: 0, left: 0, bottom: 0, zIndex: 100 }}>
        <div style={{ padding: '20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 38, height: 38, borderRadius: 12, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 800, color: '#fff', flexShrink: 0 }}>R</div>
          <div>
            <div style={{ fontSize: '1.1rem', fontWeight: 800, fontFamily: 'Outfit, sans-serif', background: 'linear-gradient(135deg, #818cf8, #22d3ee)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Realstate</div>
            <div style={{ fontSize: '0.65rem', color: '#64748b', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em' }}>User Dashboard</div>
          </div>
        </div>
      ...
      <div style={{ flex: 1, marginLeft: sidebarOpen ? 260 : 0, transition: 'margin-left 0.3s ease', display: 'flex', flexDirection: 'column' }}>
        <header style={{ height: 64, background: 'rgba(15,23,42,0.8)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 700, color: '#fff' }}>{user?.name?.[0]?.toUpperCase()}</div>
