  ...
  return (
    <div style={{ height: 'calc(100vh - 120px)', display: 'grid', gridTemplateColumns: '300px 1fr', gap: 24 }}>
      ...
      <div className="glass" style={{ overflowY: 'auto' }}>
        <h3 style={{ padding: 20, margin: 0, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>Chats</h3>
        {loading ? (
          ...
          chats.map((chat) => {
            const otherName = chat.buyer?._id === user?._id ? chat.agent?.name : chat.buyer?.name;
            return (
              <div 
                key={chat._id} 
              ...
              >
                <div style={{ fontWeight: 600, color: '#f1f5f9' }}>{otherName}</div>
