...
const UserWishlist = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const { data } = await API.get('/properties/my/wishlist');
  ...
  return (
    <div>
      <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#f1f5f9', marginBottom: 24, fontFamily: 'Outfit', display: 'flex', alignItems: 'center', gap: 12 }}>
        <HiHeart style={{ color: '#ef4444' }} /> Saved Properties
      </h1>
      {properties.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: 'rgba(255,255,255,0.02)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.06)' }}>
          <HiHeart style={{ fontSize: '3rem', color: '#475569', marginBottom: 12 }} />
          <p style={{ color: '#94a3b8', fontSize: '1.1rem' }}>You haven't saved any properties yet.</p>
          <p style={{ color: '#64748b', fontSize: '0.9rem' }}>Click the heart icon on any property to save it here.</p>
        </div>
      ) : (
